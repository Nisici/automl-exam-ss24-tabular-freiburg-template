...
from tabpfn.scripts.tabular_metrics import r2_metric
import pandas as pd
import numpy as np
...
class CAAFEClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(
        ...
        logging.info("Model: {}".format(self.base_classifier))
        ds = [
            "dataset",
            X,
            y,
        ...
        if disable_caafe:
            ...
            self.code, prompt, messages = generate_features(
                ...
                metric_used=r2_metric,
                iterative_method=self.base_classifier,
                display_method="",
                n_splits=self.n_splits,
                n_repeats=self.n_repeats,
            )
        logging.info("CODE: {}".format(self.code))
        ...
        self.classes = None
        try:
            if isinstance(self.base_classifier, XGBRegressor):
                df_train = df_train.drop(target_name, axis=1)
                df_train = make_dataset_numeric(df_train, None)
                self.base_classifier.fit(df_train, y)
            else:
                self.base_classifier.fit(df_train)
        except AssertionError:
            self.base_classifier.fit_extra('default')
